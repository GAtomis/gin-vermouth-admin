 ...
 * @LastEditTime: 2024-05-31 11:13:25
            ...
            <a-input v-model:value="formState.code" size="large" placeholder="4422">
              ...
              <template #addonAfter>
                ...
                <div v-if="captcha">
                  ...
                  <img :src="captcha.picPath " class="captcha" @click="changeCaptcha()" />
                </div>
...
const handleSubmit = async ({ username, password, code }: any) => {
  ...
  await useUser()
    .login({ username, password,captcha:code,openCaptcha:captcha.value!.openCaptcha,captchaId:captcha.value!.captchaId})
    .finally(() => {
      loading.value = false
      changeCaptcha()
    })
...
#login-warp {
  ...
  .captcha {
    height: 30px;
    background-color: rgb(195 212 242 / 1 ) !important;
    &:hover {
      cursor: pointer;
    }
  }
